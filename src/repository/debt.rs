...
pub async fn create(db: &Database, debt: Debt) -> Result<ObjectId, String> {
    let coll = db.collection::<mongodb::bson::Document>("debts");
    let doc = to_document(&debt).map_err(|e| e.to_string())?;
...
pub async fn get_by_id(db: &Database, user_id: ObjectId, id: ObjectId) -> Result<Option<Debt>, String> {
    let coll = db.collection::<mongodb::bson::Document>("debts");
    let doc = coll
    ...
    match doc {
        Some(d) => {
            let debt: Debt = from_document(d).map_err(|e| e.to_string())?;
            Ok(Some(debt))
        }
        None => Ok(None),
    }
...
pub async fn get_all(db: &Database, user_id: ObjectId) -> Result<Vec<Debt>, String> {
    let coll = db.collection::<mongodb::bson::Document>("debts");
    let mut cursor = coll
    ...
    while cursor.advance().await.map_err(|e| e.to_string())? {
        ...
        let d: Debt = from_document(doc).map_err(|e| e.to_string())?;
        out.push(d);
    }
    Ok(out)
}
